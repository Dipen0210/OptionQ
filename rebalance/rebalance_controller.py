...
from strategies.options_selector import select_option_contract
...
def run_rebalance_cycle(
    ...
    # ---------------------------------------------------------------------
    # OPTION CONTRACT SELECTION (Phase 2 Integration)
    # ---------------------------------------------------------------------
    # Convert the target weights (new_allocation) into specific option contracts
    # based on the strategy rules.
    
    option_portfolio_rows = []
    
    # We need current prices to calculate Greeks and quantities.
    # Use the last available price from price_history.
    
    for _, row in new_allocation.iterrows():
        ticker = row.get("Ticker")
        weight = float(row.get("Weight", 0.0))
        if weight <= 1e-4 or not ticker:
            continue
            
        allocated_amt = effective_capital * weight
        
        # Get underlying price
        df_price = price_history.get(ticker)
        current_price = 0.0
        if df_price is not None and not df_price.empty:
            # Try 'Close', fallback to 'date' sorting
            if "Close" in df_price.columns:
                current_price = df_price["Close"].iloc[-1]
            elif isinstance(df_price, pd.Series):
                 current_price = df_price.iloc[-1]
        
        # Default to stock if price missing or 0
        if current_price <= 0:
            continue
        ...
        try:
            ...
            if not chain.empty:
                ...
                chain_greeks = enrich_chain_with_greeks(chain, current_price)
                # 3. Select Strategy-based Contract
                selected_contract = select_option_contract(strategy_spec.name, chain_greeks)
        except Exception:
            selected_contract = None

        if selected_contract:
            # Found an option
            # Price is usually 'ask' or 'lastPrice'
            opt_price = selected_contract.get("ask", 0.0)
            if opt_price == 0.0:
                 opt_price = selected_contract.get("lastPrice", 0.0)
            
            if opt_price > 0:
                qty = int(allocated_amt / (opt_price * 100))
                # If capital too small for 1 contract, we might skip or fallback
                # For now, let's allow 0 qty to show "Insuff Capital" in UI or similar behavior
                
                option_portfolio_rows.append({
                    "Ticker": ticker,
                    "Symbol": selected_contract["contractSymbol"],
                    "AssetType": "Option",
                    "OptionType": selected_contract["type"],
                    "Strike": selected_contract["strike"],
                    "Expiration": selected_contract["expiration"],
                    "Price": opt_price,
                    "Quantity": qty,
                    "MarketValue": qty * opt_price * 100,
                    "Weight": weight,
                    "Delta": selected_contract["delta"],
                    "Gamma": selected_contract["gamma"],
                    "Vega": selected_contract["vega"],
                    "Theta": selected_contract["theta"],
                    "IV": selected_contract["effective_iv"],
                })
                continue
        
        # Fallback to Stock
        shares = int(allocated_amt / current_price)
        option_portfolio_rows.append({
            "Ticker": ticker,
            "Symbol": ticker,
            "AssetType": "Stock",
            ...
            "Price": current_price,
            "Quantity": shares,
            "MarketValue": shares * current_price,
            "Weight": weight,
            "Delta": 1.0, "Gamma": 0.0, "Vega": 0.0, "Theta": 0.0, "IV": 0.0
        })

    if option_portfolio_rows:
        final_portfolio_df = pd.DataFrame(option_portfolio_rows)
    else:
        # If no options selected (empty allocation), keep empty or fallback
        final_portfolio_df = pd.DataFrame(columns=[
            "Ticker", "Symbol", "AssetType", "OptionType", "Strike", 
            "Expiration", "Price", "Quantity", "MarketValue", 
            "Weight", "Delta", "Gamma", "Vega", "Theta", "IV"
        ])
