...
from pricing.engine import enrich_chain_with_greeks
from pricing.payoff import payoff_diagram
...
# ---------------------------------------------------------
st.set_page_config(page_title="OptionQ", layout="wide")
st.title("OptionQ: Automated Portfolio Manager")
...
ASSET_CLASS_OPTIONS = ["Equities", "Forex", "Commodities"]
...
commodities_selected = asset_class == "Commodities"
if not equities_selected:
    ...
    elif commodities_selected:
        st.sidebar.info("Commodities workflow is coming soon. Switch to Equities for the full configuration experience.")
        st.info("🛢️ Commodities trading support is under development. Select Equities to access the full workflow.")
...
if run_button:
    ...
    if results is not None:
        # Separate strategy winners (for display) from actual portfolio holdings (weights/options)
        # The controller now returns 'sector_rankings' for the strategy view
        # and 'final_portfolio' (executed_portfolio) as the actual executed positions (options or stock)
        final_df = results.get("sector_rankings")
        if final_df is None or (isinstance(final_df, dict) and not final_df):
            # Fallback if old controller version or empty dict
            final_df = results.get("final_portfolio", pd.DataFrame())
        elif isinstance(final_df, dict):
            # Flatten dict of DFs into single DF
            frames = [df for df in final_df.values() if not df.empty]
            final_df = pd.concat(frames, ignore_index=True) if frames else pd.DataFrame()
        
        if final_df.empty:
             final_df = results.get("final_portfolio", pd.DataFrame())

        executed_portfolio = results.get("final_portfolio", pd.DataFrame())
    
        candidate_pool = results["candidate_pool"]
        missing = results.get("missing_tickers", [])
        ...
        with tab1:
            ...
            if executed_portfolio is not None and not executed_portfolio.empty:
                st.divider()
                st.write("### 📜 Executed Positions (Options/Stocks)")
                
                # Check if we have option columns
                is_option_portfolio = "OptionType" in executed_portfolio.columns
                
                disp_port = executed_portfolio.copy()
                
                # Format numeric columns
                numeric_cols = ["Price", "StartPrice", "MarketValue", "Delta", "Gamma", "Vega", "Theta", "IV", "Weight"]
                for c in numeric_cols:
                    if c in disp_port.columns:
                        disp_port[c] = pd.to_numeric(disp_port[c], errors="coerce")
                        
                # Format currencies
                for c in ["Price", "MarketValue"]:
                    if c in disp_port.columns:
                        disp_port[c] = disp_port[c].apply(format_currency)
                
                # Format Greeks/Weight
                for c in ["Delta", "Gamma", "Vega", "Theta", "Weight"]:
                     if c in disp_port.columns:
                        disp_port[c] = truncate_series(disp_port[c], decimals=4)
                
                # Reorder columns for clarity
                desired_cols = [
                    "Ticker", "Symbol", "AssetType", "OptionType", 
                    "Strike", "Expiration", "Quantity", "Price", 
                    "MarketValue", "Weight", "Delta", "Gamma", "IV"
                ]
                final_cols = [c for c in desired_cols if c in disp_port.columns]
                # Add any others at the end
                existing = set(final_cols)
                remaining = [c for c in disp_port.columns if c not in existing]
                
                st.dataframe(disp_port[final_cols + remaining], hide_index=True)
                
                if is_option_portfolio:
                    st.caption("Positions include computed Greeks (Delta, Gamma, etc.) based on real-time option chain data.")
        ...
        with tab1:
            ...
            with tab6:
                st.write("### 🔎 Option Chain Inspector")
                st.info("View real-time option chains and calculated Greeks for your portfolio candidates.")
                
                # Get list of tickers from the final portfolio or candidate pool
                inspector_tickers = []
                if final_portfolio_df is not None and not final_portfolio_df.empty:
                    inspector_tickers = sorted(final_portfolio_df["Ticker"].unique().tolist())
                elif candidate_pool_df is not None and not candidate_pool_df.empty:
                    inspector_tickers = sorted(candidate_pool_df["ticker"].unique().tolist())
                
                if inspector_tickers:
                    col_insp1, col_insp2 = st.columns([1, 3])
                    with col_insp1:
                        selected_ticker = st.selectbox("Select Ticker", inspector_tickers)
                    
                    if selected_ticker:
                        # Fetch chain
                        with st.spinner(f"Fetching option chain for {selected_ticker}..."):
                            chain_df = get_option_chain(selected_ticker)
                            
                        if not chain_df.empty:
                            # Enrich with Greeks
                            # Need underlying price
                            current_px = 0.0
                            if price_history and selected_ticker in price_history:
                                px_data = price_history[selected_ticker]
                                if "Close" in px_data.columns:
                                     current_px = px_data["Close"].iloc[-1]
                                elif isinstance(px_data, pd.Series):
                                     current_px = px_data.iloc[-1]
                            
                            if current_px > 0:
                                chain_greeks = enrich_chain_with_greeks(chain_df, current_px)
                                
                                st.success(f"Loaded {len(chain_greeks)} contracts for {selected_ticker} (Ref Price: ${current_px:.2f})")
                                ...
                                display_chain = chain_greeks.copy()
                                if type_filter:
                                    display_chain = display_chain[display_chain["type"].isin(type_filter)]
                                display_chain = display_chain[display_chain["openInterest"] >= min_oi]
                                
                                # Format for display
                                disp_cols = [
                                    "contractSymbol", "strike", "type", "expiration", 
                                    "lastPrice", "bid", "ask", "impliedVolatility", 
                                    "delta", "gamma", "vega", "theta"
                                ]
                                # Filter only existing columns
                                disp_cols = [c for c in disp_cols if c in display_chain.columns]
                                
                                st.dataframe(display_chain[disp_cols], hide_index=True, use_container_width=True)
                                
                                st.divider()
                                st.write("### 📐 Payoff Diagram")
                                st.caption("Select a contract to visualize its P/L profile at expiration.")
                                
                                # Create a selection list
                                # Format: "SYMBOL | Strike | Type | Ask"
                                display_chain["Label"] = display_chain.apply(
                                    lambda x: f"{x['contractSymbol']} | {x['strike']} {x['type'].upper()} | ${x['ask']:.2f}", 
                                    axis=1
                                )
                                contract_labels = display_chain["Label"].tolist()
                                selected_label = st.selectbox("Select Contract", contract_labels)
                                
                                col_p1, col_p2 = st.columns(2)
                                with col_p1:
                                    qty_input = st.number_input("Quantity (+Long / -Short)", value=1, step=1)
                                
                                if selected_label:
                                    # Parse back the symbol/strike/etc from the label or find in DF
                                    # Easier to find in DF
                                    sel_row = display_chain[display_chain["Label"] == selected_label].iloc[0]
                                    
                                    prem = float(sel_row["ask"])
                                    if prem == 0: prem = float(sel_row["lastPrice"])
                                    
                                    direction = "long" if qty_input >= 0 else "short"
                                    
                                    if st.button("Generate Diagram"):
                                        leg = {
                                            "strike": float(sel_row["strike"]),
                                            "option_type": str(sel_row["type"]),
                                            "position": direction,
                                            "premium": prem,
                                            "quantity": abs(qty_input)
                                        }
                                        
                                        payoff_df = payoff_diagram([leg], n_points=200)
                                        ...
                                        if not payoff_df.empty:
                                            fig_payoff = px.line(
                                                ...
                                                x="UnderlyingPrice", 
                                                y="Payoff", 
                                                title=f"Payoff at Expiration: {sel_row['contractSymbol']} ({qty_input}x)",
                                                labels={"Payoff": "Profit / Loss ($)", "UnderlyingPrice": "Stock Price"}
                                            )
                                            # Add reference line for current price
                                            fig_payoff.add_vline(x=current_px, line_dash="dash", line_color="gray", annotation_text="Current Price")
                                            # Add 0-line
                                            fig_payoff.add_hline(y=0, line_color="black", line_width=1)
                                            
                                            st.plotly_chart(fig_payoff, use_container_width=True)
                            else:
                                st.warning(f"Could not determine underlying price for {selected_ticker} to calculate Greeks.")
                        else:
                            st.warning(f"No option chain found for {selected_ticker} (or no suitable expiration > 15 days).")
                else:
                    st.warning("No tickers available in the current portfolio/pool to inspect.")
